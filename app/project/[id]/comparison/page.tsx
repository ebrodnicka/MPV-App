...
export default function ComparisonPage() {
  ...
  const bestScore =
    scoreRows.length > 0
      ? Math.max(...scoreRows.map((row) => row.totalScore))
      : 0;

  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f4f8fc]">
        <p className="text-[#60788A]">Calculating comparison...</p>
      </main>
    );
  }
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mt-10">
          ...
          <p className="mt-3 max-w-3xl text-[#60788A]">
            Compare products using normalized parameter values and the
            weights defined in Configuration. The total weighted score is
            presented on a 0–100 scale.
          </p>
        </div>
        ...
        {!error && scoreRows.length === 0 && (
          <div className="mt-7 rounded-2xl border border-[#dbe8f2] bg-white p-8 text-center">
            <p className="font-bold text-[#153A5B]">
              Comparison cannot be calculated yet.
            </p>

            <p className="mt-2 text-sm text-[#60788A]">
              Make sure Configuration and Input Matrix contain complete
              data.
            </p>
          </div>
        )}
          ...
          <>
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-[#dbe8f2] bg-white p-5">
                <p className="text-sm text-[#60788A]">Products compared</p>
              ...
              <div className="rounded-2xl border border-[#dbe8f2] bg-white p-5">
                <p className="text-sm text-[#60788A]">Best score</p>

                <p className="mt-2 text-3xl font-bold text-[#153A5B]">
                  {bestScore.toFixed(1)}
                </p>

                <p className="mt-1 text-xs text-[#60788A]">out of 100</p>
              </div>
            </div>
            ...
            <div className="mt-8 space-y-4">
              {scoreRows.map((row, index) => (
                <div
                  key={row.productId}
                  className="rounded-3xl border border-[#dbe8f2] bg-white p-6 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h4 className="mt-1 text-2xl font-bold text-[#153A5B]">
                        {row.productName}
                      </h4>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-1 text-3xl font-bold text-[#153A5B]">
                        {row.totalScore.toFixed(1)}
                      </p>

                      <p className="text-xs text-[#60788A]">/ 100</p>
                    </div>
                  </div>

                  <div className="mt-5 h-3 overflow-hidden rounded-full bg-[#e8f0f6]">
                    <div
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {parameters.map((parameter) => {
                      ...
                      return (
                        <div
                          key={parameter.id}
                          className="rounded-xl bg-[#f8fbfd] p-4"
                        >
                          <p className="text-sm font-semibold text-[#153A5B]">
                            {parameter.name}
                          </p>

                          <p className="mt-2 text-lg font-bold text-[#153A5B]">
                            {rawValue ?? '—'}{' '}
                            <span className="text-sm font-normal text-[#60788A]">
                              {parameter.unit || ''}
                            </span>
                          </p>

                          <p className="mt-2 text-xs text-[#60788A]">
                            Weight: {parameter.weight}/10
                          </p>

                          <p className="mt-1 text-xs font-semibold text-[#4EA3E3]">
                            Contribution:{' '}
                            {(
                              row.parameterScores[parameter.id] || 0
                            ).toFixed(1)}{' '}
                            pts
                          </p>
                        </div>
            ...
            <div className="mt-8 overflow-x-auto rounded-3xl border border-[#dbe8f2] bg-white shadow-sm">
